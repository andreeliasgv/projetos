# 🏢 Projeto PostgreSQL — EMPRESA

Este projeto contém a modelagem e criação do banco de dados **EMPRESA** utilizando PostgreSQL.
O objetivo é representar a estrutura organizacional de uma empresa, incluindo funcionários, departamentos, dependentes, projetos e telefones.
...
* Chave primária composta (CPF_DEPENDENTE + CPF_FUNCIONARIO).
...
* Cada departamento possui um gerente.
* O gerente é um funcionário (FK para FUNCIONARIO).
...
Tabela associativa entre FUNCIONARIO e PROJETOS.

* Representa quais funcionários trabalham em quais projetos.
* Armazena a quantidade de horas trabalhadas.
* Relacionamento N:N.
...
* FUNCIONARIO → TELEFONES_FUNCIONARIO (1:N)
* FUNCIONARIO → DEPENDENTES (1:N)
* DEPARTAMENTO → PROJETOS (1:N)
* FUNCIONARIO ↔ PROJETOS (N:N via PROJETOS_FUNC)
...
Projeto acadêmico para modelagem relacional utilizando PostgreSQL.
